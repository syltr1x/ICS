...
def customer():
    ...
    class ScrollableRadiobuttonFrame(customtkinter.CTkScrollableFrame):
        ...
        def get_checked_item(self):
            ...
            scrollable_radiobutton_frame = ScrollableRadiobuttonFrame(master=self, width=500, command=self.radiobutton_frame_event,
            ...
            # create footer-menu
            menu_frame = customtkinter.CTkFrame(self, corner_radius=10)
            menu_frame.grid(row=1, column=1, padx=15, pady=5)

            menu_frame_button_1 = customtkinter.CTkButton(self.menu_frame, text="Eliminar", image=self.remove_icon_image, compound="left", command=self.remove_customer)
            menu_frame_button_1.grid(row=2, column=0, padx=10, pady=10)

            menu_frame_button_2 = customtkinter.CTkButton(self.menu_frame, text="Añadir", image=self.add_icon_image, compound="left", command=self.add_customer)
            menu_frame_button_2.grid(row=2, column=1, padx=10, pady=10)

            menu_frame_button_3 = customtkinter.CTkButton(self.menu_frame, text="Actualizar", image=self.refresh_icon_image, compound="left", command=self.refresh)
        ...
        def remove_customer(self):
            ...
            id = str(logic.get_customer("name", self.scrollable_radiobutton_frame.get_checked_item().split(" ")[0], "strict"))[:-1][1:].replace("'",'"')
            id = json.loads(id)
            id = id["id"]
