...
class DataFrame(customtkinter.CTkFrame):
    def __init__(self, master, dato, command=None, **kwargs):
        ...
        self.stock_data_label = customtkinter.CTkLabel(self, text=f'{data["stock"]}').grid(row=2, column=1, padx=(5, 30), pady=(15, 5))
...
class App(customtkinter.CTkFrame):
    def __init__(self, master, command=None, **kwargs):
        ...
        with open("data/inventory.json", "r") as dF: data = dF.read(); dF.close(); data = data[:-1][1:].replace("},","}},").split("},")
        ...
        self.menu_frame_button_1 = customtkinter.CTkButton(self.menu_frame, text="Historico", image=self.clock_icon_image, compound="left", command=self.remove_item,
    ...
    def remove_item(self):
        open("data/temp/inventory.json", "w").write(open("data/inventory.json", "r").read()) # Temp Save
    ...
    def mod_product(self):
        self.mod_item_frame = ModFrame(master=self, width=420, corner_radius=10, dato=self.scrollable_radiobutton_frame.get_checked_item())
